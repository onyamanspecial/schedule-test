...
def engine(data):
    ...
    max_effects, effects, effects_sorted, effect_priorities, combinations, *_ = data
...
def test_known_profitable_combinations(engine, data):
    ...
    max_effects, effects, effects_sorted, effect_priorities, combinations, \
    effect_multipliers, ingredient_prices, drug_types, strain_data, \
    meth_qualities, quality_names, quality_costs, drug_pricing = data
    ...
    for drug_type, depth, options, expected_path, expected_effects in known_combinations:
        ...
        if drug_type == 'marijuana':
            ...
            initial_effects = [strain_data[strain][0]]  # Base effect of the strain
        ...
        units = calculate_units(drug_type, grow_tent, pgr, drug_pricing['production_units'])
        constants = drug_pricing['constants']
        cost_formula = drug_pricing['cost_calculations'][drug_type]['formula']
        ...
        result = find_best_path(
            ...
            effect_multipliers, ingredient_prices, effect_priorities,
            initial_effects
        )
